...
/*            modified files to compile for boards        */
...
static unsigned char send_cmd(void)
{
  ...
  MMC_PORT |= 1<<WIZ_CS;       //WIZ Chip Select -> High (deactivate WIZ chip);
	MMC_PORT_CS &= ~(1<<MMC_CS); //MMC Chip Select -> Low (activate mmc)
...
//XXX	MMC_DDR |= 1<<SPI_CLK | 1<<SPI_MOSI | 1<<MMC_CS;	//MMC Chip Select -> Output
	MMC_DDR |= 1<<SPI_CLK | 1<<SPI_MOSI | 1<<WIZ_CS;	//WIZ Chip Select -> Output
	MMC_DDR_CS |= 1<<MMC_CS;	//MMC Chip Select -> Output
//XXX Block Wiznet Chip when bootloading
		MMC_PORT |= 1<<WIZ_CS; //MMC Chip Select -> High (deactivate mmc);
	...
	for (i=0; i<MMC_CMD0_RETRY; i++)
	{
		...
		MMC_PORT_CS |= 1<<MMC_CS; //MMC Chip Select -> High (deactivate mmc);
      	 	spi_send_byte(0xFF);
		if (res == 0x01)
			break;
	}
...
//was to SHORT for some of my cards !
	while(send_cmd() != 0) {
		MMC_PORT_CS |= 1<<MMC_CS; //MMC Chip Select -> High (deactivate);
		spi_send_byte(0xFF);
	}
